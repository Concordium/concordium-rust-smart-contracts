...
fn receive_mint<S: HasStateApi>(
    ...
    let mut owner_token_map =
        state.token_state.entry(params.owner).or_insert_with(|| builder.new_map());

    let mut token_count = owner_token_map.entry(params.token_id).or_insert(0);
...
mod tests {
    ...
    fn test_receive() {
        ...
        // macro.
        let mut root_entry = state_api.create_entry(&[]).expect("Creating root entry failed");
