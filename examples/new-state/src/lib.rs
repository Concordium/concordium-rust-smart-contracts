...
fn receive_mint<S: HasState>(
    ...
    host.state_mut().maybe_box = MaybeBox::WithBox(host.state_builder().new_box("I'm boxed".into()));
...
mod tests {
    ...
    fn test_receive() {
        ...
        let mut state_builder = StateBuilderTest::open(state_api.clone());
        ...
        let (_rv, state_to_store) =
            init(&InitContextTest::empty(), &mut state_builder).expect("Init failed");
        ...
        let mut host = HostTest::new_with_state_builder(state_for_rcv, state_builder);
