//! A NFT smart contract example using the Concordium Token Standard CIS3 with
...
//! Standard CIS2. In addition, a registry is
...
struct State<S> {
    ...
    /// A registry to link an account address to an public key. The public key
    /// registered here has full access of the tokens so enter a link with care.
...
impl<S: HasStateApi> State<S> {
    ...
    /// TODO comments
    fn public_key(&self, account: &AccountAddress) -> ContractResult<Option<PublicKeyEd25519>> {
        Ok(self.public_key_registry.get(account).map(|public_key| *public_key))
    }
...
#[init(contract = "cis3_nft", event = "Cis2Event<ContractTokenId, ContractTokenAmount>")]
...
/// Only the contract owner (third-party provider offering gasless transaction
/// survices) can register a public key to an account address.
#[receive(
    contract = "cis3_nft",
...
fn contract_register_public_key<S: HasStateApi>(
    ...
    // Parse the parameter.
    let params: RegisterPublicKeyParams = ctx.parameter_cursor().get()?;

    // Register the public key only if the account has no public key registered
    // already. You can only set a public key once for every account_address.
    let old_public_key =
        host.state_mut().public_key_registry.insert(params.account, params.public_key);

    // Return an error if the owner tries to update/re-write a new public key to an
...
/// Parameter type for the CIS-3 function `balanceOf` specialized to the subset
/// of TokenIDs used by this contract.
type ContractBalanceOfQueryParams = BalanceOfQueryParams<ContractTokenId>;
/// Response type for the CIS-3 function `balanceOf` specialized to the subset
...
pub struct ContractPublicKeyQueryResponse(
    #[concordium(size_length = 2)] pub Vec<Option<PublicKeyEd25519>>,
);

impl From<Vec<Option<PublicKeyEd25519>>> for ContractPublicKeyQueryResponse {
    fn from(results: Vec<Option<concordium_std::PublicKeyEd25519>>) -> Self {
...
/// Get the public keys of addresses.
///
/// It rejects if:
/// - It fails to parse the parameter.
#[receive(
...
/// Parameter type for the CIS-3 function `tokenMetadata` specialized to the
...
mod tests {
    ...
    fn test_register_public_key() {
        ...
        claim_eq!(
            public_key.unwrap(),
            Some(PUBLIC_KEY),
            "Account should have a public key registered"
        );

        // Checking that `ACCOUNT_1` has the correct public key registered and there is
        // no public key registered for `ACCOUNT_0`.
        ...
        claim_eq!(
            ...
            "ACCOUNT_1 should have the correct public key registered and ACCOUNT_0 should have no \
