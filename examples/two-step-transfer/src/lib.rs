...
mod tests {
    ...
    // /// Total reserved balance determined by active requests relative to
    // `now` fn sum_reserved_balance<S: HasStateApi>(state: &State<S>, now:
    // Timestamp) -> Amount {     state
    //         .requests
    //         .iter()
    ...
    // TransferRequestTimeToLiveMilliseconds::from_millis(10),     };
    //     let parameter_bytes = to_bytes(&parameter);

    //     TestEnv.set_parameter(0, &parameter_bytes);
    ...
    //     claim_eq!(state.requests.iter().count(), 0, "No transfer request at
    // initialisation"); }
    ...
    //     let parameter = Message::RequestTransfer(request_id, transfer_amount,
    // target_account);     let parameter_bytes = to_bytes(&parameter);

    //     let ctx = ReceiveContext::open(());
    //     TestEnv.set_parameter(0, &parameter_bytes);
    //     TestEnv.set_receive_sender(Address::Account(account1));
    //     TestEnv.set_slot_time(Timestamp::from_timestamp_millis(0));
    ...
    //         state_builder,
    //     };
    //     TestEnv.set_receive_balance(Amount::from_micro_ccd(0));

    //     let receive_amount = Amount::from_micro_ccd(100);
    //     // Execution
    //     let res = contract_receive_message(&ctx, &mut host, receive_amount);
    //     claim!(res.is_ok(), "Contract receive failed, but it should not
    // have.");
    ...
    //     claim_eq!(request.supporters.len(), 1, "Only one is supporting the
    // request from start");     claim!(request.supporters.contains(&
    // account1), "The request sender supports the request"); }
    ...
    //     let request_id = 0;
    //     let transfer_amount = Amount::from_micro_ccd(50);
    //     let parameter = Message::SupportTransfer(request_id, transfer_amount,
    // target_account);     let parameter_bytes = to_bytes(&parameter);
    ...
    //     // Execution
    //     let res: ContractResult<()> = contract_receive_message(&ctx, &mut
    // host, receive_amount);

    //     claim!(res.is_ok(), "Contract receive support failed, but it should
    // not have.:");     claim_eq!(
    //         sum_reserved_balance(host.state(),
    // Timestamp::from_timestamp_millis(0)),
    //         Amount::from_micro_ccd(50),
    ...
    //     );
    //     let request = host.state().requests.get(&request_id).unwrap();
    //     claim_eq!(request.supporters.len(), 2, "Two should support the
    // transfer request");     claim!(request.supporters.contains(&
    // account2), "The support sender supports the request"); }
    ...
    //     // Create Request with id 0, to transfer 50 to target_account
    //     let parameter =
    //         Message::RequestTransfer(request_id_active, transfer_amount,
    // target_account);     let parameter_bytes = to_bytes(&parameter);
    ...
    //             .metadata()
    //             .slot_time()
    //             .checked_sub(init_params.transfer_request_ttl) // In the
    // past.             .expect_report("Checked sub for times_out_at
    // failed."),         supporters: supporters.clone(),
    //     });
    ...
    //             .metadata()
    //             .slot_time()
    //             .checked_add(init_params.transfer_request_ttl) // In the
    // future.             .expect_report("Checked sub for times_out_at
    // failed."),         supporters,
    //     });
    ...
    //     let res = host.with_rollback(|host| contract_receive_message(&ctx,
    // host, Amount::zero()));     claim_eq!(res,
    // Err(ReceiveError::RequestAlreadyExists));

    //     // Check that both initial requests still exist in the state since it
    // was rolled     // back.
    ...
    // TransferRequestTimeToLiveMilliseconds::from_millis(10),     };
    //     let mut supporters = BTreeSet::new();
    //     // we add one more account to `supporters` so with the sender account
    // the number     // of supporters reaches the threshold
    //     supporters.insert(account1);
    //     let request = TransferRequest {
    //         supporters,
    //         target_account,
    ...
    //     let res: ContractResult<()> = contract_receive_message(&ctx, &mut
    // host, ccd_amount);     // The contract execution shouldn't fail and
    // the sum of reserved balances should     // be zero
    //     res.is_ok()
    //         && (sum_reserved_balance(host.state(),
    // Timestamp::from_timestamp_millis(0))             ==
    // Amount::from_micro_ccd(0)) }
