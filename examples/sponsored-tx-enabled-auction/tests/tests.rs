...
fn test_add_item() {
    ...
    let rv: ItemState = invoke
        .parse_return_value()
        .expect("ViewItemState return value");
...
fn full_auction_flow_with_cis3_permit_function() {
    ...
    assert_eq!(
        balance_of_alice_and_auction_contract.0,
        [TokenAmountU64(100), TokenAmountU64(0)]
    );

    // Create input parameters for the `permit` transfer function.
    let additional_data = AdditionalDataIndex { item_index: 1u16 };
    ...
    chain
        .tick_block_time(Duration::from_millis(100000))
...
fn full_auction_flow_with_several_bids() {
    ...
    assert_eq!(
        balance_of_alice_and_auction_contract.0,
        [TokenAmountU64(100), TokenAmountU64(2)]
    );
    ...
    assert_eq!(
        balance_of_alice_and_auction_contract.0,
        [TokenAmountU64(102), TokenAmountU64(0)]
    );
}
