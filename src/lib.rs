 ...
 *  * Once users are done with their off-chain transactions, the validator can settle the transactions by adding a settlement to the chain using [contract_receive_add_settlement]. A settlement consists of two `Vec`s of  addresses and amounts specifying which addresses have to pay which amounts and which addresses receive which amounts, respectively. Adding settlements can only be added by the validator to prevent DoS attacks.
